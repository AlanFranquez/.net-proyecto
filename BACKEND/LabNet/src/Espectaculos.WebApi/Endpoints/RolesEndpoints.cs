...
public static class RolesEndpoints
{
    public static IEndpointRouteBuilder MapRolesEndpoints(this IEndpointRouteBuilder endpoints)
    {
        ...
        }).WithName("ListarRoles").WithTags("Roles").WithOpenApi();
        
        
        group.MapDelete("/", async ([FromBody] DeleteRolCommand command, [FromServices]IMediator mediator) =>
        {
            await mediator.Send(command);
            return Results.NoContent();
        }).WithName("EliminarRoles").WithTags("Roles");
        
        group.MapPost("/", async (CreateRolCommand command, IMediator mediator) =>
        {
            var id = await mediator.Send(command);
            return Results.Ok(id);
        }).WithName("CrearRol").WithTags("Roles");
        
        group.MapPut("/{id:guid}", async (Guid id, UpdateRolCommand cmd, IMediator mediator) =>
        {
            cmd.RolId = id; // Asegura que el id de la ruta se copie al comando
