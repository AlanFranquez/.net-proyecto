...
using Espectaculos.Application.Sincronizaciones.Queries.ListarSincronizaciones;
using MediatR;
...
{
    public static IEndpointRouteBuilder MapSincronizacionEndpoints(this IEndpointRouteBuilder endpoints)
    {
        var group = endpoints.MapGroup("sincronizaciones");

        group.MapGet("/", async ([FromServices] IMediator mediator) =>
        {
            var query = new ListarSincronizacionesQuery();
            var sincronizaciones = await mediator.Send(query);

            return Results.Ok(sincronizaciones);
        }).WithName("ListarSincronizaciones").WithTags("Sincronizaciones").WithOpenApi();
        
        
        group.MapDelete("/", async ([FromBody] DeleteSincronizacionCommand command, [FromServices]IMediator mediator) =>
        {
            await mediator.Send(command);
            return Results.NoContent();
        }).WithName("EliminarSincronizacion").WithTags("Sincronizaciones");
        
        group.MapPost("/", async (CreateSincronizacionCommand command, IMediator mediator) =>
        {
            var id = await mediator.Send(command);
            return Results.Ok(id);
        }).WithName("CrearSincronizacion").WithTags("Sincronizaciones");
        
        group.MapPut("/{id:guid}", async (Guid id, UpdateSincronizacionCommand cmd, IMediator mediator) =>
        {
            cmd.SincronizacionId = id; // Asegura que el id de la ruta se copie al comando
            var updatedId = await mediator.Send(cmd);
            return Results.Ok(updatedId);
        })
        .WithName("EditarSincronizaciones")
        .WithTags("Sincronizaciones");
