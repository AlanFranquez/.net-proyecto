...
public static class EventosAccesosEndpoints
{
    public static IEndpointRouteBuilder MapEventosAccesosEndpoints(this IEndpointRouteBuilder endpoints)
    {
        var group = endpoints.MapGroup("eventos");

        group.MapGet("/", async ([FromServices] IMediator mediator) =>
        {
            var query = new ListarEventosQuery();
            var eventos = await mediator.Send(query);

            return Results.Ok(eventos);
        }).WithName("ListarEventos").WithTags("Eventos").WithOpenApi();
        
        
        group.MapDelete("/", async ([FromBody] DeleteEventoCommand command, [FromServices]IMediator mediator) =>
        {
            await mediator.Send(command);
            return Results.NoContent();
        }).WithName("EliminarEvento").WithTags("Eventos");
        
        group.MapPost("/", async (CreateEventoCommand command, IMediator mediator) =>
        {
            var id = await mediator.Send(command);
            return Results.Ok(id);
        }).WithName("CrearEvento").WithTags("Eventos");
        
        group.MapPut("/{id:guid}", async (Guid id, UpdateEventoCommand cmd, IMediator mediator) =>
        {
            cmd.EventoId = id; // Asegura que el id de la ruta se copie al comando
